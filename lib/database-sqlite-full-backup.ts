import { getPlatformDatabase, MySQLDatabase, executeCompatibleQuery, executeCompatibleSingle, executeCompatibleRun, convertSQLiteDateToMySQL } from './platform-database'
...
// MySQL 表已通过迁移脚本创建，不需要初始化表结构
// initializeTables, runMigrations, initializeDefaultPlatforms 等函数已移除
...
export class SmsRecordDB {
  ...
  async findByOutId(outId: string): Promise<SmsRecord | null> {
    const sql = 'SELECT * FROM sms_records WHERE out_id = ?'
    return await executeCompatibleSingle<SmsRecord>(sql, [outId])
  }
  ...
  async findAll(limit = 100, offset = 0): Promise<SmsRecord[]> {
    const sql = `
      ...
      LIMIT ? OFFSET ?
    `
    return await executeCompatibleQuery<SmsRecord>(sql, [limit, offset])
  }
  
  // 根据手机号查询记录
  async findByPhoneNumber(phoneNumber: string, limit = 50): Promise<SmsRecord[]> {
    const sql = `
      SELECT * FROM sms_records 
      WHERE phone_number = ? 
      ORDER BY created_at DESC 
      LIMIT ?
    `
    return await executeCompatibleQuery<SmsRecord>(sql, [phoneNumber, limit])
  }
  
  // 根据状态查询记录
  async findByStatus(status: string, limit = 100, offset = 0): Promise<SmsRecord[]> {
    const sql = `
      SELECT * FROM sms_records 
      WHERE status = ? 
      ORDER BY created_at DESC 
      LIMIT ? OFFSET ?
    `
    return await executeCompatibleQuery<SmsRecord>(sql, [status, limit, offset])
  }
  ...
  // 查询可重发的记录
  async findResendableRecords(limit = 100, offset = 0): Promise<SmsRecord[]> {
    const sql = `
      SELECT * FROM sms_records 
      WHERE status IN ('发送失败', '发送中(已停止查询)') 
      OR (status = '发送中' AND created_at < DATE_SUB(NOW(), INTERVAL 10 MINUTE))
      ORDER BY created_at DESC 
      LIMIT ? OFFSET ?
    `
    return await executeCompatibleQuery<SmsRecord>(sql, [limit, offset])
  ...
  async count(): Promise<number> {
    const sql = 'SELECT COUNT(*) as count FROM sms_records'
    const result = await executeCompatibleSingle<{ count: number }>(sql)
    return result?.count || 0
  }

  // 根据状态统计记录数
  async countByStatus(status: string): Promise<number> {
    const sql = 'SELECT COUNT(*) as count FROM sms_records WHERE status = ?'
    const result = await executeCompatibleSingle<{ count: number }>(sql, [status])
    return result?.count || 0
  }

  // 根据手机号统计记录数
  async countByPhoneNumber(phoneNumber: string): Promise<number> {
    const sql = 'SELECT COUNT(*) as count FROM sms_records WHERE phone_number = ?'
    const result = await executeCompatibleSingle<{ count: number }>(sql, [phoneNumber])
    return result?.count || 0
  }
  
  // 删除记录
  async deleteRecord(id: number): Promise<boolean> {
    const sql = 'DELETE FROM sms_records WHERE id = ?'
    const result = await executeCompatibleRun(sql, [id])
    return result.changes > 0
  }
  ...
  async cleanupOldRecords(daysToKeep = 30): Promise<number> {
    ...
    `
    const result = await executeCompatibleRun(sql, [daysToKeep])
